...
// There are probably 3 recommendations: {
// Let's just find the EN one and clean it up.
const enIndex = content.indexOf('EN: {');
let beforeEN = content.substring(0, enIndex);
let afterEN = content.substring(enIndex);

// Let's replace any multiple recommendations: { inside afterEN with just one.
// The easiest way is to use regex to remove ALL recommendations blocks in afterEN, 
// and then re-insert it right before admin: {
